...
func _ready():
	...
	rand_x.seed = seed_x
	rand_x.randomize()
	x_direction = rand_x.randf_range(-0.75,0.75)
...
func _on_area_2d_body_entered(body):
	if body.is_in_group("Player"):
		get_parent().get_node("PowerUp1Audio").play()
		body.health += 400
		body.upgrade1_aquired = true
		DisplayServer.window_set_title('Max health increased. "Lookin\' fresh there my friend"')
